...
:::caution
`window.grafanaBootData` is not through the plugin, but through Grafana itself. This means it can easily change based on the Grafana version.
:::
...
The easiest way to see what's useful is to just log it.

```js
console.log(window.grafanaBootData);
```

In no particular order, here's a few things that can be gotten, _that I think is useful,_ from `window.grafanaBootData`.
