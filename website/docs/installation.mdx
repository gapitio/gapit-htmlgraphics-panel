...
import { htmlGraphics } from '/config.js';
// Code must be a variable (or imported) or newlines doesn't work
const latestGithubReleaseDockerComposeCode = () =>
  `version: '3'
services:
  grafana:
...
networks:
  grafana:
    name: grafana`;
const specificDockerComposeCode = () =>
  `version: '3'
services:
  grafana:
    image: grafana/grafana
...
:::info
For a more detailed explaination on how to install plugins, check out <https://grafana.com/docs/grafana/latest/plugins/installation/>
:::
...
:::info
More information about plugin catalog <https://grafana.com/docs/grafana/latest/plugins/catalog/>
:::
...
:::info
More information about grafana-cli <https://grafana.com/docs/grafana/latest/administration/cli/>
:::
...
<CodeBlock className="language-yaml">{latestGithubReleaseDockerComposeCode()}</CodeBlock>
...
<CodeBlock className="language-yaml">{specificDockerComposeCode()}</CodeBlock>
